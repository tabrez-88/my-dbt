...
WITH branch_office AS (
    SELECT 
        {{ decode_base64(external_id) }} AS office_external_id,
        id AS office_id
    FROM {{ ref('m_office') }}
),
roles AS (
...
SELECT 
    ...
    {{ decode_base64(u.USERNAME) }} AS display_name,
    {{ decode_base64(u.MOBILEPHONE1) }} AS mobile_no,
    {{ decode_base64(u.EMAIL) }} AS email_address,
    {{ decode_base64(u.ENCODEDKEY) }} as external_id,
